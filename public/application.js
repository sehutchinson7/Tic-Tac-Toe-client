...
var currentPlayer = 'X'; // Define currentPlayer to start as X
...
var patchObject = {
  ...
  $('.cell').on('click', function (event) {
    ...
    if ($(event.target).text() === '') {
      ...
      var id = event.target.id; // Add currentPlayer to JS board array
...
var onTieGame = function onTieGame() {
  $('.playerMove').html("It's a draw!");
  $('#game-board').hide();
};

var onWinner = function onWinner() {
...
var onUpdateGameData = function onUpdateGameData() {
...
var updateGameData = function updateGameData(data) {
  return $.ajax({
...
var getGameDataSuccess = function getGameDataSuccess(data) {
  $('.total').text('');
...
var onNewGame = function onNewGame(event) {
  ...
  $('.playerMove').html('Player 1 Start Game');
  game.newGame();
  api.startGame().then(ui.newGameSuccess);
};

var onGetGameData = function onGetGameData() {
  event.preventDefault();
  api.getGameData().then(ui.getGameDataSuccess);
  // .catch(console.error)
};
